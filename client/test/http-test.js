...
describe('http - EC2와 S3 설정을 확인합니다.', () => {
  ...
  it('3. EC2와 S3 사이의 환경설정을 올바르게 설정해야 합니다.', async () => {
    const result = await axios.get('https://yeoraebucket.s3.ap-northeast-2.amazonaws.com/static/js/2.2e1a1952.chunk.js');
    const ec2url = result.data.includes(`${process.env.REACT_APP_API_URL}`);
